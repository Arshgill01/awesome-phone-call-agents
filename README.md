...
- [`exact-ref`](skills/exact-ref/) - Write-gate for an exact identifier captured by a CALL-E call (confirmation, ticket, off-hire reference): compares intended vs extracted character by character, names the provenance (`spoken_only`, `conversational_confirmed`, `mismatch`, `independently_verified`), and opens the write only after a human types the value from a second channel. Zero-dependency `scripts/exactref.mjs`; no calls placed.
